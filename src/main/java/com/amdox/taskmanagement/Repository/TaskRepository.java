...
public interface TaskRepository extends JpaRepository<TaskEntity, Long> {
    ...
    Optional<TaskEntity> findTaskEntityByAssignedUserAndCreatedBy(UserEntity user, UserEntity admin);

    boolean existsByTitle(String title);

    @Query("SELECT t FROM TaskEntity t WHERE t.assignedUser =: userEntity")
    Optional<TaskEntity> findTaskEntityByAssignedUser(UserEntity userEntity);
